        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-mono text-[13px] text-[#aaa]">#{issue.number}</span>
              <Link href={`/${user}/${repo}/issues/${issue.number}`}>
                <h3 className="text-[14px] font-medium text-[#111] hover:text-[#666] transition-colors truncate">{issue.title}</h3>
              </Link>
            </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className={`text-[12px] font-semibold uppercase tracking-[0.5px] ${issue.state === 'open' ? 'text-[#34a853]' : 'text-[#888]'}`}>
              {issue.state}
            </span>
