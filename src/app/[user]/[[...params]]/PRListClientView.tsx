        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-mono text-base text-[#aaa]">#{pr.number}</span>
              <Link href={`/${user}/${repo}/pulls/${pr.number}`}>
                <h3 className="text-base font-medium text-[#111] hover:text-[#666] transition-colors truncate">{pr.title}</h3>
              </Link>
              {pr.draft && (
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className={`text-[13px] font-semibold uppercase tracking-[0.5px] ${pr.state === 'open' ? 'text-[#34a853]' : 'text-[#888]'}`}>
              {pr.state}
            </span>
