      ...
      <div className="w-[90%] max-w-[800px] mx-auto pt-12 pb-20">
        ...
        {activeTab === 'cli' && (
          <div className="space-y-8">
            <div>
              <div className="text-[11px] text-[#aaa] font-semibold tracking-[1.5px] uppercase mb-2">Terminal Shortcuts</div>
              ...
              {[
                ...
                <div key={item.cmd} className="bg-[#f8f9fa] py-[14px] px-[16px]" style={{ borderLeft: `3px solid ${item.color}` }}>
                  <div className="flex items-center gap-3 mb-1">
                    <code className="text-[13px] font-mono font-semibold text-[#111]">{item.cmd}</code>
                    <span className="text-[13px] text-[#333]">{item.desc}</span>
                  </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="text-[11px] text-[#aaa] font-semibold tracking-[1.5px] uppercase">Installation</div>
              ...
              <div className="mt-3 bg-[#f8f9fa] py-[10px] px-[14px] text-[12px] text-[#666]" style={{ borderLeft: '3px solid #f59e0b' }}>
                Run <code className="bg-[#eee] px-1 py-0.5 rounded text-[11px]">source ~/.bashrc</code> or restart your terminal to activate.
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'api' && (
          <div className="space-y-8">
            <div>
              <div className="text-[11px] text-[#aaa] font-semibold tracking-[1.5px] uppercase mb-2">REST API</div>
              <p className="text-[14px] text-[#666] leading-[1.6]">
            ...
            <div className="bg-[#f8f9fa] py-[14px] px-[16px]" style={{ borderLeft: '3px solid #111' }}>
              ...
              <div className="text-[13px] text-[#666] mb-2">Include your API key in requests:</div>
            ...
            <div>
              <div className="text-[11px] text-[#aaa] font-semibold tracking-[1.5px] uppercase mb-4">Endpoints</div>
              <div className="space-y-[2px]">
                {endpoints.map((endpoint, idx) => (
                  <div key={idx} className="bg-[#f8f9fa] py-[14px] px-[16px]" style={{ borderLeft: '3px solid #34a853' }}>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[11px] font-semibold uppercase tracking-[1px] text-[#34a853]">{endpoint.method}</span>
                      <code className="font-mono text-[13px] text-[#111]">{endpoint.path}</code>
                    </div>
                    <div className="text-[13px] text-[#666] mb-2">{endpoint.description}</div>
                    <pre className="bg-[#1a1a1a] text-[#ddd] rounded px-3 py-2 font-mono text-[11px] overflow-x-auto">
                      {endpoint.example}
                    </pre>
                  </div>
                ))}
            ...
            <Link href="/api-docs" className="text-[13px] text-[#888] hover:text-[#111] transition-colors inline-block">
        ...
        {activeTab === 'mcp' && (
          <div className="space-y-8">
            <div>
              <div className="text-[11px] text-[#aaa] font-semibold tracking-[1.5px] uppercase mb-2">Model Context Protocol</div>
              <p className="text-[14px] text-[#666] leading-[1.6]">
            ...
            <div>
              <div className="text-[11px] text-[#aaa] font-semibold tracking-[1.5px] uppercase mb-4">What you'll be able to do</div>
              <div className="space-y-[2px]">
                {[
