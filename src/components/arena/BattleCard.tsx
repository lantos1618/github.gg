    ...
    <div data-testid="arena-battle-card" className="border-b border-[#f0f0f0] last:border-0">
      <div className="py-5">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              <span className="text-[13px] font-semibold uppercase tracking-[1px]" style={{ color: statusColor }}>
          ...
          <div className="text-right">
            <div className="text-base font-semibold uppercase tracking-[0.5px]" style={{ color: statusColor }}>
              {isPending ? '...' : statusLabel}
            </div>
            <div className="text-[13px] text-[#aaa] font-mono mt-0.5">
        ...
        {battle.criteria && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {battle.criteria.map((criterion) => (
              <span key={criterion} className="px-2 py-0.5 bg-[#f8f9fa] text-[13px] text-[#888] border border-[#eee] rounded capitalize">
                {criterion.replace('_', ' ')}
              </span>
            ))}
          </div>
        ...
        <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
          <CollapsibleTrigger className="w-full flex items-center justify-between py-2 text-base font-medium text-[#888] hover:text-[#111] transition-colors">
          ...
          <CollapsibleContent className="mt-3 space-y-4">
            {battle.scores && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-[13px] text-[#aaa] font-semibold tracking-[1px] uppercase mb-1">Challenger</div>
                  <div className="text-[25px] font-semibold text-[#111]">{battle.scores.challenger.total.toFixed(1)}</div>
                </div>
                <div>
                  <div className="text-[13px] text-[#aaa] font-semibold tracking-[1px] uppercase mb-1">Opponent</div>
                  <div className="text-[25px] font-semibold text-[#111]">{battle.scores.opponent.total.toFixed(1)}</div>
                </div>
              </div>
            )}
                ...
                <table className="w-full text-base border-collapse">
                  <thead>
                    <tr className="border-b border-[#ddd]">
                      <td className="py-1.5 text-[13px] text-[#aaa] font-semibold">Criterion</td>
                      <td className="py-1.5 text-[13px] text-[#aaa] font-semibold text-right">You</td>
                      <td className="py-1.5 text-[13px] text-[#aaa] font-semibold text-right">Opp</td>
                    </tr>
                  </thead>
                  <tbody>
            ...
            {battle.aiAnalysis && (
              <div className="bg-[#f8f9fa] py-[14px] px-[16px]" style={{ borderLeft: '3px solid #4285f4' }}>
                <div className="text-[13px] font-semibold uppercase tracking-[1px] text-[#4285f4] mb-1">AI Verdict</div>
