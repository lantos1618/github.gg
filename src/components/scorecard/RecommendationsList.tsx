...
const CATEGORY_ICONS: Record<string, React.ReactNode> = {
  ...
  maintainability: <Wrench className="w-4 h-4 text-blue-500" />,
  documentation: <FileText className="w-4 h-4 text-emerald-500" />,
};
...
const CATEGORY_BGS: Record<string, string> = {
  security: 'bg-rose-50/50 dark:bg-rose-900/10 hover:border-rose-200 dark:hover:border-rose-800',
  performance: 'bg-amber-50/50 dark:bg-amber-900/10 hover:border-amber-200 dark:hover:border-amber-800',
  maintainability: 'bg-blue-50/50 dark:bg-blue-900/10 hover:border-blue-200 dark:hover:border-blue-800',
  documentation: 'bg-emerald-50/50 dark:bg-emerald-900/10 hover:border-emerald-200 dark:hover:border-emerald-800',
};

const DEFAULT_ICON = <Lightbulb className="w-4 h-4 text-gray-500" />;
const DEFAULT_BG = 'bg-gray-50/50 dark:bg-gray-900/10 hover:border-gray-200 dark:hover:border-gray-800';
      ...
      <CardContent className="p-0">
        <div className="divide-y divide-gray-100 dark:divide-gray-800">
          {sortedRecommendations.map((rec, index) => (
            <div
              key={index}
              className={`p-6 transition-all duration-200 border-l-4 border-l-transparent hover:border-l-primary ${CATEGORY_BGS[rec.category] || DEFAULT_BG}`}
            >
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="flex items-center gap-2 px-2.5 py-1 rounded-md bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300">
                      {CATEGORY_ICONS[rec.category] || DEFAULT_ICON}
                      <span className="capitalize">{rec.category}</span>
                    </div>
                    <Badge variant="outline" className={`capitalize font-normal ${PRIORITY_COLORS[rec.priority]}`}>
