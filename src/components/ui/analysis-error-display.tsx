    ...
    return (
      <div className={`text-center py-8 ${className}`}>
        <div className="max-w-md mx-auto mb-8">
          ...
          <p className="text-[14px] text-[#666] mb-4">{config.description}</p>
          <div className="bg-[#f8f9fa] py-[14px] px-[16px] text-left" style={{ borderLeft: '3px solid #f59e0b' }}>
            ...
            <div className="text-[14px] text-[#333] leading-[1.6]">{config.explanation}</div>
          </div>
        </div>
        <UpgradePrompt />
      </div>
          ...
          <div className="flex gap-2 justify-center">
            {onRetry && (
              <button
                onClick={onRetry}
                disabled={isPending}
                className="px-4 py-2 bg-[#111] text-white text-[14px] font-medium rounded-md hover:bg-[#333] transition-colors disabled:opacity-50"
              >
            ...
            {onRetryWithContext && hasPreviousResult && (
              <button
                ...
                disabled={isPending}
                className="px-4 py-2 bg-[#f8f9fa] text-[#333] text-[14px] font-medium rounded-md border border-[#ddd] hover:border-[#aaa] transition-colors disabled:opacity-50"
              >
