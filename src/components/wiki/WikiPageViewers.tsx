  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 text-[13px] text-[#aaa] hover:text-[#111] transition-colors cursor-pointer"
      >
        ...
        <div className="flex flex-col h-full">
          <div className="flex items-start justify-between p-6 border-b border-[#eee]">
            <div>
              ...
              <div className="text-[28px] font-semibold text-[#111]">
                {totalViewCount}
              </div>
              <div className="flex items-center gap-3 text-[12px] text-[#aaa] mt-1">
          ...
          <div className="flex-1 overflow-y-auto p-6">
                    ...
                    return (
                      <div key={viewer.userId} className="flex items-start gap-3 py-2">
                        <Avatar className="h-8 w-8 flex-shrink-0">
                          ...
                          <AvatarFallback className="text-[12px] bg-[#f8f9fa] text-[#aaa]">{viewer.username?.[0]?.toUpperCase()}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <a href={`/${viewer.username}`} className="text-[14px] font-medium text-[#111] hover:text-[#666] truncate" onClick={() => setIsOpen(false)}>
                              {viewer.username}
                            </a>
                            {isRecent && (
            ...
            {guestViews > 0 && (
              <div>
                ...
                <div className="bg-[#f8f9fa] py-[12px] px-[14px] text-[13px] text-[#666]" style={{ borderLeft: '3px solid #ccc' }}>
            ...
            {registeredUserCount === 0 && guestViews === 0 && (
              <div className="py-12 text-center">
                <p className="text-[14px] text-[#aaa]">No views yet</p>
              </div>
            )}
          </div>
        </div>
