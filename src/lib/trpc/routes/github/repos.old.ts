  ...
  getReposForScrollingWithUser: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        // Get user repos with limit to avoid fetching ALL repos (performance optimization)
        const limitedUserRepos = await githubService.getUserRepositories(undefined, input.limit);
